...
{
    public async Task<IDataResult<List<GetPropertyResponseDto>>> GetAsPaginatedListAsync(GetPropertiesRequestDto dto)
    {
        var sql = @"
          SELECT
              p.id_property AS Id, 
        ...
        var properties = await propertyRepository.ExecuteRawSqlAsync<GetPropertyResponseDto>(sql);
        return new SuccessDataResult<List<GetPropertyResponseDto>>(properties, EMessages.Success.Translate());
    }

    public async Task<IDataResult<GetPropertyResponseDto>> GetAsync(int id)
    {
        var sql = $@"
          SELECT
              p.id_property AS Id, 
        ...
        var property = await propertyRepository.ExecuteRawSqlSingleAsync<GetPropertyResponseDto>(sql);
        if (property == null) {
            return new ErrorDataResult<GetPropertyResponseDto>(property, EMessages.DataNotFound.Translate());
        }
        return new SuccessDataResult<GetPropertyResponseDto>(property, EMessages.Success.Translate());
    }

    public async Task<bool> IsPropertyExistbyId(int id)
    {
        return await propertyRepository.AnyAsync(p => p.IdProperty == id);
    }
}
